...
def display(name,domain):
    if domain=="hotels":
        ...
        buckets={'ambience':[],'amenities':[],'cleanliness':[],'facility':[],'food':[],'price':[],'staff':[]}
    else:
        df=pd.read_csv('final_airline.csv')
        df_temp=df[df['Airline']==name]
        buckets={'ambience':[],'amenities':[],'cleanliness':[],'checkin':[],'food':[],'price':[],'service':[]}
...
def main():
    ...
    categories={"hotels":['Cleanliness','Amenities','Facility','Food','Staff','Ambience','Price'],"airlines":['Cleanliness','Amenities','Checkin','Food','Service','Flight Ambience','Price']}
    if args.domain == 'hotels':
        ...
        if df["Hotel Name"].str.contains(str(args.hotel_name)).any():
            ...
            final_df=pd.DataFrame(columns=["Hotel Name","Review","Review_Lemma","Keyword","Sentiment","User Contribution","Recency","joy","sadness","fear","disgust","anger","category"])
        ...
        if df["Airline"].str.contains(str(args.airline)).any():
            ...
            print("Review Categorization complete")
            final_df=pd.DataFrame(columns=["Airline","Review","From","To","Area","Class","Review_Lemma","Keyword","Sentiment","User Contribution","Recency","joy","sadness","fear","disgust","anger","category"])
            for i in categories[args.domain]:
                df=pd.read_csv(str(i)+".csv")
                df['category']=str(i)
                final_df=final_df.append(df,sort=False)
